...
interface YogaBruleCalmTimerProps {
  exercise: Exercise;
  onComplete: () => void;
  onClose: () => void;
}

// Phases Yoga Brûle-Calm basées sur les étapes du JSON
const YOGA_BRULE_CALM_PHASES = [
  { name: '2 Salutations au soleil', duration: 180, description: 'Debout → flexion avant → planche → cobra → chien tête en bas → revient debout.', color: Colors.morning },
  { name: 'Flow guerrier', duration: 240, description: 'Guerrier 1 → Guerrier 2 → Triangle. 30 s par posture, chaque côté.', color: Colors.agpBlue },
  { name: 'Planche douce', duration: 120, description: '2×30 s planche bras tendus, 30 s de repos entre les deux.', color: Colors.agpGreen },
  { name: 'Chien tête en bas', duration: 120, description: 'Tenues de 20 s × 3 avec respiration calme.', color: Colors.relaxation },
  { name: 'Assis respiration 4-6', duration: 120, description: 'Inspire 4 s, expire 6 s.', color: Colors.agpBlue },
  { name: 'Relaxation courte', duration: 120, description: 'Allongé, relâche tout le corps.', color: Colors.agpGreen },
];

const { width, height } = Dimensions.get('window');

export default function YogaBruleCalmTimer({ exercise, onComplete, onClose }: YogaBruleCalmTimerProps) {
  const { addCompletedExercise, user } = useAuth();
  const [currentPhaseIndex, setCurrentPhaseIndex] = useState(0);
  const [timeLeft, setTimeLeft] = useState(YOGA_BRULE_CALM_PHASES[0].duration);
  const [isRunning, setIsRunning] = useState(false);
  const [isCompleted, setIsCompleted] = useState(false);
  const [totalTimeElapsed, setTotalTimeElapsed] = useState(0);
  const intervalRef = useRef<number | null>(null);

  const currentPhase = YOGA_BRULE_CALM_PHASES[currentPhaseIndex];
  const totalDuration = YOGA_BRULE_CALM_PHASES.reduce((sum, phase) => sum + phase.duration, 0);
  ...
  const handlePhaseComplete = () => {
    ...
    if (currentPhaseIndex < YOGA_BRULE_CALM_PHASES.length - 1) {
      const nextPhaseIndex = currentPhaseIndex + 1;
      setCurrentPhaseIndex(nextPhaseIndex);
      setTimeLeft(YOGA_BRULE_CALM_PHASES[nextPhaseIndex].duration);
      
      const nextPhase = YOGA_BRULE_CALM_PHASES[nextPhaseIndex];
  ...
  const completeWorkout = async () => {
    ...
    try {
      ...
      if (result.success) {
        Alert.alert(
          '🧘‍♀️ Yoga Brûle-Calm terminé !',
          `Namaste ! Vous avez complété "${exercise.titre}". Votre corps est énergisé et votre esprit apaisé !`,
          [
            {
              text: 'Continuer',
              onPress: () => {
      ...
      console.error('💥 Erreur enregistrement Yoga Brûle-Calm:', error);
    }
  };
  ...
  const resetTimer = () => {
    ...
    setTimeLeft(YOGA_BRULE_CALM_PHASES[0].duration);
    ...
    <View style={styles.container}>
      ...
      >
          ...
          <View style={styles.progressCircle}>
            ...
            <View style={styles.innerCircle}>
              ...
              <Text style={styles.phaseCounter}>
                {currentPhaseIndex + 1}/{YOGA_BRULE_CALM_PHASES.length}
              </Text>
            </View>
          </View>
        </View>
          ...
          <View style={[styles.phaseCard, { borderLeftColor: currentPhase.color }]}>
            ...
            <View style={styles.yogaTip}>
              <Text style={styles.yogaTipText}>
                🧘‍♀️ Flow fluide qui brûle et apaise à la fois
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.overallProgressContainer}>
          <Text style={styles.overallProgressTitle}>Progression Yoga Brûle-Calm</Text>
        ...
        <View style={styles.phasesListContainer}>
          <Text style={styles.phasesListTitle}>Programme Yoga Brûle-Calm complet :</Text>
          {YOGA_BRULE_CALM_PHASES.map((phase, index) => (
